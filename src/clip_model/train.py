...
# TODO: можно попробовать вместо слоя Transformer обучать простой MLP но без дообучения GPT

# TODO: добавить метрики качества, оценить ими результаты на val и test
...
class MemeDataset(Dataset):
    ...
    def __init__(self, prefix_length: int, data_path: str, images_path: str, gpt2_type: str = GPT_MODEL,
                 normalize_prefix=False, clip_model: str = CLIP_MODEL, max_len: int = 512):

        self.images_path = images_path
        self.prefix_length = prefix_length
        self.normalize_prefix = normalize_prefix
        clip, processor = ruclip.load(clip_model, device='cpu')
        self.clip_processor = ruclip.Predictor(clip, processor, device='cpu', bs=1)
        captions = []
        self.image_ids = []
        ...
        self.max_seq_len = min(max_seq_len, max_len)
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, ...]:
        ...
        image = Image.open(f"{self.images_path}/{image_id}.jpg")
...
class ClipCaptionModel(nn.Module):
    ...
    def __init__(self, prefix_length: int, clip_length: Optional[int] = None, prefix_size: int = 512,
        ...
        self.prefix_length = prefix_length
        # TODO: проверить загрузку модели гпт тут
...
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', default='data/for_training/train.csv')
    ...
    dataset = MemeDataset(prefix_length, data_path=args.data, images_path=args.images_path,
